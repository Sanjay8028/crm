...
if($_GET['did']!='') 
{
  ...
  $sql = "DELETE FROM tblusers WHERE id='".$_GET['did']."'";
...
for($i=0;$i<count($count1);$i++)
{
  ...
  $sql = "Update tblusers set assign_to='".$_POST['manager']."' WHERE id='$del_id'";
  $result2 = $db->query($sql);
}
if($result2)
{
    echo "<script>alert('Manager Assign  Successfully !');</script>";
}
}
?>
...
<header class="panel_header">
    <h2 class="title float-left btn btn-danger">All Team Leader</h2>
...
<?php
$queryus ="SELECT *FROM tblusers where user_type='Manager'";
...
<button type="submit" class="btn btn-success btn-mt-20 btn-width-100" name="ass" onSubmit="return validate();">Assign</button>
  ...
  <div class="display-data-table-area">
    ...
    <tbody>
        <?php
        $queryu ="SELECT *FROM tblusers where user_type='Team Leader'";
        ...
        <td class="clr-blue"><input name="checkbox[]" type="checkbox" id="checkbox[]" value="<?php echo $row['id']; ?>" class="checkBoxClass"></td>
            ...
            <td><b style="font-size:14px"><a href="view-business-consultant.php?uid=<?php echo $row['username']; ?>" style="color:#3F51B5 !important; background:none;"><?php echo $row['name']; ?></a></b></td>
            ...
            <a href="update-empolyee.php?id=<?php echo $row['id'];?>"><i class="fa fa-boo" title="Read More"><img src="https://i.pinimg.com/originals/d9/62/3f/d9623fc777b7498ab50cb37520975ef5.png" style="width:30px;float:left"></i></a>
             ...
             <a href="all-teamleader.php?did=<?php echo $row['id'];?>"><i class="fa fa-boo" title="Read More"><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSqdV_TgwjyVgB6KDmAJGN3oT9XTME0sflargbMBwODuYwJA19RDXRS0aDEtDkXmZdL2aU&usqp=CAU" style="width:30px;float:left"></i></a>
             
             <a href="all-teamleader.php?sid=<?php echo $row['id'];?>&status=<?php echo $row['status']?>" class="btn btn-info">Status </a>
            </td>
        </tr>
        <?php
        $i++;
