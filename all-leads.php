...
{
    $quet = "Select *FROM tblusers WHERE username='".$_POST['telecaller']."'";
    $rst = $db->query($quet);
    $row=mysqli_fetch_array($rst);
    
    $quetm = "Select *FROM tblusers WHERE username='".$row['assign_to']."'";
    $rstm = $db->query($quetm);
    $rowm=mysqli_fetch_array($rstm);

  $del_id=$_POST['checkbox'][$i];
  $sql = "Update tblleads set assign_to='".$_POST['telecaller']."', assign_date='".date('d-m-Y')."', to_teamleader='".$row['assign_to']."', to_manager='".$rowm['assign_to']."' WHERE id='$del_id'";
...
if($result2)
{
    $sql = "Update tblrequest_lead set status='Done', ustatus='1', assign_date='".date('d-m-Y h:i:s')."',assign_qty='".$tt."',assigns_date='".date('d-m-Y')."' WHERE username='".$_POST['telecaller']."' and ustatus='' and status='Pending'";
    $result2 = $db->query($sql);
...
<header class="panel_header">
    <h2 class="title float-left btn btn-danger">All Parties</h2>
...
<div class="col-lg-2 col-md-6 col-12">
<a href="" onclick="Export()" class="btn btn-success btn-mt-20 btn-width-100">Export</a>
</div>
</div>
<br/>
<div class="row">
...
<div class="table-responsive">
    ...
    <tbody>
        ...
        <td class="clr-blue"><input name="checkbox[]" type="checkbox" id="checkbox[]" value="<?php echo $row['id']; ?>" class="checkBoxClass"></td>
			...
			<a href="history.php?id=<?php echo $row['id'];?>"><i class="fa fa-histo" title="History"> <img src="https://e7.pngegg.com/pngimages/496/295/png-clipart-logo-graphics-computer-icons-encapsulated-postscript-icon-design-history-icon-text-trademark.png" style="width:30px;float:left"></i></a>
            </td>
        </tr>
        <?php
        $i++;
    ...
    <div class="modal-dialog my-modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
       	  <span class="modal-title">All Parties</span>
