...
if(isset($_POST['ass'])) 
{
    ...
    {
        $que .="to_manager='".$_POST['mid']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }
    if($_POST['mid']=='' && $_POST['tid']=='' && $_POST['bid']!='')
    {
        $que .="assign_to='".$_POST['bid']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }
    if($_POST['mid']=='' && $_POST['tid']!='' && $_POST['bid']=='')
    {
        $que .="to_teamleader='".$_POST['tid']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }

    if($_POST['mid']!='' && $_POST['tid']!='' && $_POST['bid']!='')
    {
        $que .="to_manager='".$_POST['mid']."' and to_teamleader='".$_POST['tid']."' and assign_to='".$_POST['bid']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }

    if($_POST['mid']!='' && $_POST['tid']!='' && $_POST['bid']=='')
    {
        $que .="to_manager='".$_POST['mid']."' and to_teamleader='".$_POST['tid']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }

    if($_POST['mid']=='' && $_POST['tid']!='' && $_POST['bid']!='')
    {
        $que .="to_teamleader='".$_POST['tid']."' and assign_to='".$_POST['bid']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }

    if($_POST['mid']!='' && $_POST['tid']=='' && $_POST['bid']!='')
    {
        $que .="to_manager='".$_POST['mid']."' and assign_to='".$_POST['bid']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }
...
else
{
    ...
    {
        $queryu ="SELECT *FROM tblleads where f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
        $result = $db->query($queryu);
    }
}
...
if(isset($_POST['search'])) 
{
    ...
    {
        $que .="company_name='".$_POST['name']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }
    if($_POST['name']=='' && $_POST['email']=='' && $_POST['mobile']!='')
    {
        $que .="mobile='".$_POST['mobile']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }
    if($_POST['name']=='' && $_POST['email']!='' && $_POST['mobile']=='')
    {
        $que .="email='".$_POST['email']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }

    if($_POST['name']!='' && $_POST['mobile']!='' && $_POST['email']!='')
    {
        $que .="company_name='".$_POST['name']."' and mobile='".$_POST['mobile']."' and email='".$_POST['email']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }

    if($_POST['name']!='' && $_POST['mobile']!='' && $_POST['email']=='')
    {
        $que .="company_name='".$_POST['name']."' and mobile='".$_POST['mobile']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }

    if($_POST['name']=='' && $_POST['mobile']!='' && $_POST['email']!='')
    {
        $que .="mobile='".$_POST['mobile']."' and email='".$_POST['email']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }

    if($_POST['name']!='' && $_POST['mobile']=='' && $_POST['email']!='')
    {
        $que .="company_name='".$_POST['name']."' and email='".$_POST['email']."' and f_date NOT IN ('".date('Y-m-d')."','') and status IN ('Interested','Follow-Up')";
    }
    $queryu ="SELECT *FROM tblleads where ".$que."";
    $result = $db->query($queryu);
}
    ...
    <section class="wrapper main-wrapper" style=''>
        ...
        <section class="box">
            <header class="panel_header">
                <h2 class="title float-left btn btn-danger">TOTAL FOLLOW-UPS</h2>
                	...
                	<div class="col-12 col-md-12 col-lg-12">                    	
                        <form method="POST" action="" enctype="multipart/form-data">
                        	<div class="row pb-25 pt-10">
                                ...
                                <div class="col-lg-3 col-md-3 col-12">
                                    <button type="submit" class="btn btn-success btn-mt-20 btn-width-100" name="ass">Search</button>
                                </div>
...
<script type="text/javascript">
  $(document).on('click', '.getid', function(){  
   var id = $(this).attr("id");
   $.ajax({  
        url:"ajax/get_totalfollowups.php",  
        method:"POST",  
        data:{id:id},    
    ...
    <div class="modal-dialog my-modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
       	  <span class="modal-title">TOTAL FOLLOW-UPS</span>
