...
if(isset($_POST['ass'])) 
{
    ...
    {
        $que .="to_manager='".$_POST['mid']."' and assigns_date='".date('d-m-Y')."' and status=''";
    }
    if($_POST['mid']=='' && $_POST['tid']=='' && $_POST['bid']!='')
    {
        $que .="assign_to='".$_POST['bid']."' and assigns_date='".date('d-m-Y')."' and status=''";
    }
    if($_POST['mid']=='' && $_POST['tid']!='' && $_POST['bid']=='')
    {
        $que .="to_teamleader='".$_POST['tid']."' and assigns_date='".date('d-m-Y')."' and status=''";
    }

    if($_POST['mid']!='' && $_POST['tid']!='' && $_POST['bid']!='')
    {
        $que .="to_manager='".$_POST['mid']."' and to_teamleader='".$_POST['tid']."' and assign_to='".$_POST['bid']."' and status=''";
    }

    if($_POST['mid']!='' && $_POST['tid']!='' && $_POST['bid']=='')
    {
        $que .="to_manager='".$_POST['mid']."' and to_teamleader='".$_POST['tid']."' and status=''";
    }

    if($_POST['mid']=='' && $_POST['tid']!='' && $_POST['bid']!='')
    {
        $que .="to_teamleader='".$_POST['tid']."' and assign_to='".$_POST['bid']."' and status=''";
    }

    if($_POST['mid']!='' && $_POST['tid']=='' && $_POST['bid']!='')
    {
        $que .="to_manager='".$_POST['mid']."' and assign_to='".$_POST['bid']."' and status=''";
    }
...
else
{
    ...
    {
        $queryu ="SELECT *FROM tblleads where status=''";
        $result = $db->query($queryu);
    }
}
...
if(isset($_POST['search'])) 
{
    ...
    {
        $que .="company_name='".$_POST['name']."' and status=''";
    }
    if($_POST['name']=='' && $_POST['email']=='' && $_POST['mobile']!='')
    {
        $que .="mobile='".$_POST['mobile']."' and status=''";
    }
    if($_POST['name']=='' && $_POST['email']!='' && $_POST['mobile']=='')
    {
        $que .="email='".$_POST['email']."' and status=''";
    }

    if($_POST['name']!='' && $_POST['mobile']!='' && $_POST['email']!='')
    {
        $que .="company_name='".$_POST['name']."' and mobile='".$_POST['mobile']."' and email='".$_POST['email']."' and status=''";
    }

    if($_POST['name']!='' && $_POST['mobile']!='' && $_POST['email']=='')
    {
        $que .="company_name='".$_POST['name']."' and mobile='".$_POST['mobile']."' and status=''";
    }

    if($_POST['name']=='' && $_POST['mobile']!='' && $_POST['email']!='')
    {
        $que .="mobile='".$_POST['mobile']."' and email='".$_POST['email']."' and status=''";
    }

    if($_POST['name']!='' && $_POST['mobile']=='' && $_POST['email']!='')
    {
        $que .="company_name='".$_POST['name']."' and email='".$_POST['email']."' and status=''";
    }
    $queryu ="SELECT *FROM tblleads where ".$que."";
    $result = $db->query($queryu);
}
    ...
    <section class="wrapper main-wrapper" style=''>
        ...
        <section class="box">
            <header class="panel_header">
                <h2 class="title float-left btn btn-danger">Untouch Parties</h2>
                        ...
                        <tbody>
                           <?php
                            ...
                            <td><b style="font-size:14px"><?php echo $row['company_name']; ?></b></td>
                                ...
                                <td><b style="font-size:14px"><?php echo substr($row['product'], 0, 15); ?></b></td>
...
<script type="text/javascript">
  $(document).on('click', '.getid', function(){  
   var id = $(this).attr("id");
   $.ajax({  
        url:"ajax/get_untouchparties.php",  
        method:"POST",  
        data:{id:id},    
    ...
    <div class="modal-dialog my-modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
       	  <span class="modal-title">Untouch Parties</span>
